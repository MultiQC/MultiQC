...
MultiQC does its best to have one row per sample in a table. Some modules, however, may produce multiple results per sample. These will be treated as separate samples alongside the shorter "merged" sample name from downstream steps, resulting in half-empty rows. To address this, we introduce an new configuration option:
...
It will tell MultiQC that `SAMPLE_R1` and `SAMPLE_R2` should be grouped together into `SAMPLE`. For the modules that have only data for `SAMPLE_R1` and `SAMPLE_R2` - e.g. FastQC - `SAMPLE` will be generated by merging together the data from `SAMPLE_R1` and `SAMPLE_R2`, while individual sample data can be seen by "expanding" a table row.

The way a module merges sample groups for each specific metric can be specified in the module code, e.g.:
...
For configuration, more sophisticated patterns are supported such as listing multiple strings, or using regular expression with `type: regex`:

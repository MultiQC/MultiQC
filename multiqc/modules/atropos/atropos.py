...
PASSFAILS = (
    '<script type="text/javascript">atropos_passfails_{phase} = {statuses};</script>'
)
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self, name="", anchor="", info="", from_config=True):
        ...
        super(MultiqcModule, self).__init__(
            ...
            info=info or self.submodule.get_info()
            + (
    ...
    def init_from_config(self):
        ...
        for file_dict in log_files:
            ...
            try:
                ...
                raise Exception(
                    "Error loading file {} - it may be corrupted".format(fileobj)
                )
...
class TrimModule(Submodule):
    ...
    def add_data(self, sample_id, data):
        ...
        self.atropos_general_data[sample_id]["mean_sequence_length"] = mean(
            data["derived"]["mean_sequence_lengths"]
        )
...
class QcModule(Submodule, metaclass=ABCMeta):
    ...
    def atropos_assets(self, parent):
        ...
        parent.status_colours = dict(
            (status.name, status.color) for status in (PASS, WARN, FAIL)
        )
    ...
    def atropos_general(self, parent):
        ...
        headers["percent_fails_{}".format(self.get_phase())] = {
            ...
            "description": "Percentage of Atropos QC modules failed ({}-trimming)".format(
                self.get_phase()
            ),
            "max": 100,
            "min": 0,
    ...
    def atropos_plots(self, parent):
        ...
        for section in self.qc_sections:
            ...
            context = section(
                self.atropos_qc_data, self.atropos_general_data, phase=self.get_phase()
            )
...
class Section(metaclass=ABCMeta):
    ...
    def get_read_plot(self, context, plot_data, read):
        ...
        """
        return self.plot_type.plot(
            plot_data, self.get_plot_config(context, read=read)
        )
...
class TrimmedLength(Section):
    ...
    def prepare_data(self, data, general):
        def add_lengths(
            side, read, sample_id, num_reads, adapter_length, read_dict, dest_dict
        ):
    ...
    def compute_statistic(self, context, data):
        ...
        for pos, base_counts in data["rows"].items():
            ...
            lower_quartile = weighted_lower_quantile(
                quals, (float(c) / counts_cumsum[-1] for c in base_counts), 0.25
            )
...
class PerSequenceGC(QcSection):
    ...
    def add_to_context(self, context, data):
        ...
        if tgc is not None:
            ...
            tgc_path = os.path.join(
                os.path.dirname(__file__), "atropos_theoretical_gc", tgc_fn
            )
            ...
            try:
                ...
                log.warning(
                    "Couldn't open FastQC Theoretical GC Content file %s", tgc_path
                )
    ...
    def compute_statistic(self, context, data):
        ...
        for pct in range(101):
            dev_pct += abs(
                min(context["theoretical_gc"][pct][1] * context["max_total"], maxcount)
                - data["hist"].get(pct, 0)
            )
    ...
    def get_read_plot(self, context, data, read):
        ...
        plot_data_norm = dict(
            (sample_id, values[1]) for sample_id, values in data.items()
        )
        plot_config = {
        ...
        plot_config["extra_series"][0][0]["data"] = [
            (d[0], d[1] * 100) for d in context["theoretical_gc"]
        ]
        ...
        def get_n_data(bases):
            ...
            return dict(
                n_counts=ordered_dict(
                    (int(pos), bases["rows"][pos][nidx]) for pos in bases["rows"].keys()
                ),
                total_counts=ordered_dict(
                    (int(pos), sum(bases["rows"][pos])) for pos in bases["rows"].keys()
                ),
            )
...
class NormalDistribution(object):
    ...
    def __call__(self, value):
        return (math.e ** -(((value - self.mean) ** 2) / self.sd2)) / math.sqrt(
            self.sd2 * math.pi
        )
...
def sort_hist(hist):
    return ordered_dict(
        sorted(
            ((int(length), count) for length, count in hist.items()), key=lambda x: x[0]
        )
    )
...
def get_status_cols(statuses):
    ...
    """
    return ordered_dict(
        (sample_id, status.color) for sample_id, status in statuses.items()
    )

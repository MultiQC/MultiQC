...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self, name="", anchor="", info="", from_config=True):
        ...
        super(MultiqcModule, self).__init__(
            name=name or self.submodule.get_name(),
            anchor=anchor or self.submodule.get_anchor(),
            href=ATROPOS_GITHUB_URL,
            info=info
            or self.submodule.get_info()
            + (
                "is a general-purpose NGS pre-processing tool that "
                "specializes in adapter- and quality-trimming, written "
...
class TrimModule(Submodule):
    ...
    def can_add_data(self, data):
        # The TrimModule adds the general stats even if the trim
        # command wasn't run
        return True

    def add_data(self, sample_id, data):
        super().add_data(sample_id, data)
        self.atropos_general_data[sample_id] = data["derived"].copy()
        self.atropos_general_data[sample_id]["mean_sequence_length"] = mean(data["derived"]["mean_sequence_lengths"])
        self.atropos_general_data[sample_id].update(
            dict((key, data[key]) for key in ("total_record_count", "total_bp_counts"))
        )
        ...
        if "trim" in data:
            ...
            for mod_dict in data["trim"]["modifiers"].values():
                if "fraction_records_with_adapters" in mod_dict:
                    self.atropos_general_data[sample_id]["fraction_records_with_adapters"] = mean(
...
class QcModule(Submodule, metaclass=ABCMeta):
    ...
    def atropos_assets(self, parent):
        ...
        parent.status_colours = dict((status.name, status.color) for status in (PASS, WARN, FAIL))
    ...
    def atropos_plots(self, parent):
        ...
        for section in self.qc_sections:
            section_name = "{}_{}".format(section.name, self.get_phase())
            context = section(self.atropos_qc_data, self.atropos_general_data, phase=self.get_phase())
        ...
        for sample_id, num_fails in fails.items():
            self.atropos_general_data[sample_id]["percent_fails_{}".format(self.get_phase())] = (
                num_fails * 100 / len(self.qc_sections)
            )

        parent.intro += ADD_LISTENERS.format(phase=self.get_phase())
        parent.intro += PASSFAILS.format(phase=self.get_phase(), statuses=json.dumps(simplify(statuses)))


class PreModule(QcModule):
    @classmethod
    def get_name(cls):
        return "Atropos: Pre-trim QC"
    ...
    @classmethod
    def get_info(cls):
        return "reports pre-trimming statistics. Atropos "
...
class PostModule(QcModule):
    ...
    @classmethod
    def get_info(cls):
        return "reports post-trimming statistics. Atropos "
...
SUBMODULES = dict(trim=TrimModule, pre=PreModule, post=PostModule)
...
class Section(metaclass=ABCMeta):
    ...
    statuses: get_statuses -> get_status (for each sample) -> get_status_for_pair ->
        get_status_for
...
class TrimmedLength(Section):
            ...
            for side in sorted(adapter_data.keys()):
                ...
                for read, read_data in enumerate(reads, 1):
                    if not any(read_data):
                        continue
                    adapter_plots.append(
                        linegraph.plot(
                            read_data,
                            self.get_plot_config(context, seq=adapter_seq, side=side, read=read),
                        )
                    )
    ...
    def get_plot_config(self, context, seq="", side="back", read=1):
        return {
            "id": "atropos_trimmed_length_plot_{seq}_{side}_{read}".format(seq=seq, side=side, read=read),
            "title": "Atropos: Lengths of Trimmed Sequences<br/>Seq={seq}, " "Side={side}, Read={read}".format(
...
class QcSection(Section, metaclass=ABCMeta):
    @property
    def default_thresholds(self):
        return ()

    @property
    def comparison_oper(self):
        return operator.lt
    ...
    def get_status(self, context, sample_data1, sample_data2=None):
        ...
        """
        stat1, stat2 = [
            self.compute_statistic(context, data) if data else None for data in (sample_data1, sample_data2)
        ]
    ...
    def get_status_for(self, context, stat):
        ...
        for status, threshold in zip((FAIL, WARN), self.default_thresholds):
            if self.comparison_oper(stat, threshold):
                return status
        return PASS
...
class PerBaseQuality(QcSection):
    ...
    @property
    def display(self):
        return "Sequence Quality Histograms"

    @property
    def anchor(self):
        return "atropos_base_qualities"
    ...
    def get_plot_config(self, context, read=1):
        return {
            "id": "atropos_base_qualities_plot_{phase}_{read}".format(phase=context["phase"], read=read),
    ...
    def compute_statistic(self, context, data):
        ...
        for pos, base_counts in data["rows"].items():
            ...
            lower_quartile = weighted_lower_quantile(quals, (float(c) / counts_cumsum[-1] for c in base_counts), 0.25)
...
class PerSequenceQuality(QcSection):
    ...
    def get_plot_config(self, context, read=1):
        return {
            "id": "atropos_qualities_plot_{phase}_{read}".format(phase=context["phase"], read=read),
...
<script type="text/javascript">
    ...
    def compute_statistic(self, context, data):
        ...
        for row in data["rows"].values():
            ...
            max_diff = max(max_diff, abs(frac[0] - frac[3]), abs(frac[1] - frac[2]))
        return max_diff

    def get_sample_plot_data(self, context, data):
        cols = tuple(col.lower() for col in data["columns"])
        return dict((pos, dict(zip(cols, row))) for pos, row in data["rows"].items())
    ...
    def get_read_plot(self, context, plot_data, read):
        ...
        return self.plot_html.format(phase=context["phase"], read=read, data=json.dumps(plot_data))
...
class PerSequenceGC(QcSection):
    ...
    def add_to_context(self, context, data):
        ...
        if tgc is not None:
            ...
            try:
                ...
                log.warning("Couldn't open FastQC Theoretical GC Content file %s", tgc_path)
    ...
    def compute_statistic(self, context, data):
        ...
        for pct in range(101):
            dev_pct += abs(
                min(context["theoretical_gc"][pct][1] * context["max_total"], maxcount) - data["hist"].get(pct, 0)
            )
    ...
    def get_read_plot(self, context, data, read):
        ...
        plot_config = {
            "id": "atropos_gc_plot_{phase}_{read}".format(phase=context["phase"], read=read),
        ...
        plot_config["extra_series"][0][0]["data"] = [(d[0], d[1] * 100) for d in context["theoretical_gc"]]
        plot_config["extra_series"][1][0]["data"] = [
            (d[0], d[1] * context["max_total"]) for d in context["theoretical_gc"]
        ]
...
class PerBaseN(QcSection):
    ...
    @property
    def anchor(self):
        return "atropos_per_base_N_content"
    ...
    def compute_statistic(self, context, data):
        return max(data["n_counts"][pos] / data["total_counts"][pos] for pos in data["total_counts"].keys())

    def get_plot_config(self, context, read=1):
        return {
            "id": "atropos_per_base_N_content_plot_{phase}_{read}".format(phase=context["phase"], read=read),
...
See the <a href="#general_stats">General Statistics Table</a>.</p>"""
    ...
    def get_plot_config(self, context, read=1):
        return {
            "id": "atropos_lengths_plot_{phase}_{read}".format(phase=context["phase"], read=read),
...
def hist_to_means(hist):
    ...
    return dict((int(pos), weighted_mean(hist["columns"], counts)) for pos, counts in hist["rows"].items())
...
def simplify(obj):
    ...
    if obj is None or isinstance(obj, str) or isinstance(obj, Number) or isinstance(obj, bool):
        return obj

"""MultiQC submodule to parse BBTools covhist output (coverage depth histogram)"""
...
def parse_bbtools_covhist(module: BaseMultiqcModule) -> int:
    """
    Parse BBTools covhist output files showing histogram of coverage depth levels.

    Shows the number of occurrences of each coverage depth level.
    """

    data_by_sample: Dict[str, Dict] = {}

    for f in module.find_log_files("bbtools/covhist", filehandles=True):
        parsed = _parse_covhist_file(f["f"])
        if parsed:
            s_name = f["s_name"]
            if s_name in data_by_sample:
                log.debug(f"Duplicate sample name found! Overwriting: {s_name}")
            module.add_data_source(f, s_name=s_name, section="covhist")
    ...
    pconfig = {
        "id": "bbtools-covhist-plot",
        "title": "BBTools: Coverage Histogram",
        "xlab": "Coverage (depth)",
        "xsuffix": "X",
    ...
    module.write_data_file(data_by_sample, "multiqc_bbtools_covhist")

    return len(data_by_sample)


def _parse_covhist_file(f) -> Dict:
    """
    Parse a BBTools covhist file.

    Expected columns: #Coverage, numBases
    """
    parsed_data: Dict = {}
    ...
    for line in f:
        ...
        try:
            coverage = int(parts[0].lstrip("#"))
            num_bases = int(parts[1])
            parsed_data[coverage] = [num_bases]

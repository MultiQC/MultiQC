"""MultiQC submodule to parse BBTools lhist output (read length histogram)"""
...
def parse_bbtools_lhist(module: BaseMultiqcModule) -> int:
    """
    Parse BBTools lhist output files showing read length histogram.
    """

    data_by_sample: Dict[str, Dict] = {}

    for f in module.find_log_files("bbtools/lhist", filehandles=True):
        parsed = _parse_lhist_file(f["f"])
        if parsed:
            s_name = f["s_name"]
            if s_name in data_by_sample:
                log.debug(f"Duplicate sample name found! Overwriting: {s_name}")
            module.add_data_source(f, s_name=s_name, section="lhist")
    ...
    pconfig = {
        "id": "bbtools-lhist-plot",
        "title": "BBTools: Read Lengths",
    ...
    module.write_data_file(data_by_sample, "multiqc_bbtools_lhist")

    return len(data_by_sample)


def _parse_lhist_file(f) -> Dict:
    """
    Parse a BBTools lhist file.

    Expected columns: #Length, Count
    """
    parsed_data: Dict = {}

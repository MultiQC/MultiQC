...
class MultiqcModule(BaseMultiqcModule):
    ...
    # Initialise the parent object
        ...
        self.write_data_file(self.3pGtoAfreq_data, 'multiqc_damageprofiler_3pGtoAfreq')
    ...
    def parse3pG(self, f):
        ...
        if parsed_data is not None:
            ...
            s_name = self.clean_s_name(list(parsed_data.keys())[0],f[root])

            if s_name in self.summary_data:
                log.debug("Duplicate sample name found! Overwriting: {}".format(s_name))
        
            self.3pGtoAfreq_data[s_name] = parsed_data.get(key)
        else: 

...
COV_METRICS = list(itertools.chain.from_iterable([[
    ...
    Metric('Average autosomal coverage over {}'             , 'Mean aut cov'    , None , 'hid', 'x'    , 'Average autosomal coverage over {}. Calculated as the number of bases that aligned to the autosomal loci in {} divided by the total number of loci in the autosomal loci in {}. If there is no autosomes in the reference genome, or the region does not intersect autosomes, this metric shows as NA.'),
    ...
    Metric('PCT of genome with coverage [ 20x: 50x)'        , '20x..50x'        , 'hid', 'hid', '%'    , 'Percentage of sites in region with at least 20x but less than 50x coverage.'),
    Metric('PCT of genome with coverage [ 50x:100x)'        , '50x..100x'       , 'hid', 'hid', '%'    , 'Percentage of sites in region with at least 50x but less than 100x coverage.'),
]]))

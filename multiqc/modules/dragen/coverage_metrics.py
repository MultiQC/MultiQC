...
class DragenCoverageMetrics(BaseMultiqcModule):
    def add_coverage_metrics_base_method(
        ...
        for metric in gen_stats_headers.keys():
            gen_headers[metric + " " + table_name] = gen_stats_headers[metric]
        own_headers = OrderedDict()
        for metric in own_tabl_headers.keys():
            own_headers[metric + " " + table_name] = own_tabl_headers[metric]

        self.general_stats_addcols(new_data, gen_headers, namespace=NAMESPACE)

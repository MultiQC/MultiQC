...
class MultiqcModule(BaseMultiqcModule):
    ...
    the command line used by fasterp. If you prefer, you can tell the module to use
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(
            name="fasterp",
            anchor="fasterp",
            href="https://github.com/drbh/fasterp",
            info="Blazing fast all-in-one FASTQ preprocessor.",
            extra="""
            Fasterp is an ultra-fast implementation with the same interface as fastp.
            It goes through fastq files in a folder and performs a series of quality control and filtering.
        ...
        data_by_sample = dict()
        for f in self.find_log_files("fasterp", filehandles=True):
            s_name, parsed_json = self.parse_fasterp_log(f)
            if not s_name:
                continue
        ...
        # Find and load any fasterp reports
        self.fasterp_data = dict()
        self.fasterp_duplication_plotdata = dict()
        self.fasterp_insert_size_data = dict()
        self.fasterp_all_data = dict()
        self.fasterp_qual_plotdata = dict()
        self.fasterp_gc_content_data = dict()
        self.fasterp_n_content_data = dict()
        self.fasterp_overrepresented_sequences = dict()
        for k in [
            ...
            self.fasterp_qual_plotdata[k] = dict()
            self.fasterp_gc_content_data[k] = dict()
            self.fasterp_n_content_data[k] = dict()
            self.fasterp_overrepresented_sequences[k] = dict()
        ...
        self.add_section(
            name="Filtered Reads",
            anchor="fasterp-filtered-reads-chart",
            description="Filtering statistics of sampled reads.",
            plot=self.fasterp_filtered_reads_chart(),
        )

        # Duplication rate plot
        self.add_section(
            name="Duplication Rates",
            anchor="fasterp-duprates",
            description="Duplication rates of sampled reads.",
            plot=linegraph.plot(
                self.fasterp_duplication_plotdata,
                {
                    "id": "fasterp-duprates-plot",
                    "title": "Fasterp: Duplication Rate",
        ...
        self.add_section(
            name="Insert Sizes",
            anchor="fasterp-insert-size",
            description="Insert size estimation of sampled reads.",
            plot=linegraph.plot(
                self.fasterp_insert_size_data,
                {
                    "id": "fasterp-insert-size-plot",
                    "title": "Fasterp: Insert Size Distribution",
                    "xlab": "Insert size",
                    "ylab": "Read percent",
                    "y_clipmax": 100,
                    "ymin": 0,
        ...
        self.add_section(
            ...
            anchor="fasterp-seq-quality",
            description="Average sequencing quality over each base of all reads.",
            plot=self.fasterp_read_qual_plot(),
        )

        # GC content plot
        self.add_section(
            name="GC Content",
            anchor="fasterp-seq-content-gc",
            description="Average GC content over each base of all reads.",
            plot=self.fasterp_read_gc_plot(),
        )

        # N content plot
        self.add_section(
            name="N content",
            anchor="fasterp-seq-content-n",
            description="Average N content over each base of all reads.",
            plot=self.fasterp_read_n_plot(),
        )

        # Overrepresented sequences plot
        self.add_section(
            name="Overrepresented Sequences",
            anchor="fasterp-overrepresented-sequences",
            description="Overrepresented sequences in the reads.",
            plot=self.fasterp_overrepresented_sequences_plot(),
        )

        # Save entire original parsed JSON
        self.write_data_file(self.fasterp_all_data, "multiqc_fasterp")

    def parse_fasterp_log(self, f) -> Tuple[Optional[str], Dict]:
        """Parse the JSON output from fasterp and save the summary statistics"""
        try:
            parsed_json = json.load(f["f"])
        except json.JSONDecodeError as e:
            log.warning(f"Could not parse fasterp JSON: '{f['fn']}': {e}, skipping sample")
            return None, {}
        ...
        if isinstance(config.use_filename_as_sample_name, list):
            ...
            should_use_filename = True
        elif getattr(config, "fasterp", {}).get("s_name_filenames", False):
            # Deprecated option - warn user
            log.warning(
                "The 'fasterp.s_name_filenames' config option is deprecated. Use the global 'use_filename_as_sample_name' option instead."
            )
            should_use_filename = True
        ...
        if s_name is None:
            ...
            # "fasterp -c -g -y -i Sample 1 1.fastq.gz -o ..."
            # "fasterp -c -g -y --in1 Sample 1 1.fastq.gz --out1 ..."
            # "fasterp -c -g -y --in1 Sample 1 1.fastq.gz --in2 Sample 1_2.fastq.gz --out1 ..."
            #
            ...
            if m:
                ...
                s_name = f["s_name"]
                log.warning(
                    f"Could not parse sample name from the fasterp command:\n{cmd}\n"
    ...
    def process_parsed_data(self, parsed_json: Dict, s_name: str):
        ...
        self.fasterp_data[s_name] = {}
        self.fasterp_duplication_plotdata[s_name] = {}
        self.fasterp_insert_size_data[s_name] = {}
        self.fasterp_all_data[s_name] = parsed_json
        for k in [
            "read1_before_filtering",
            "read2_before_filtering",
            "read1_after_filtering",
            ...
            self.fasterp_qual_plotdata[k][s_name] = {}
            self.fasterp_gc_content_data[k][s_name] = {}
            self.fasterp_n_content_data[k][s_name] = {}

        # Parse filtering_result
        try:
            for k in parsed_json["filtering_result"]:
                self.fasterp_data[s_name][f"filtering_result_{k}"] = float(parsed_json["filtering_result"][k])
        except KeyError:
            log.debug(f"fasterp JSON did not have 'filtering_result' key: '{s_name}'")

        # Parse duplication
        try:
            self.fasterp_data[s_name]["pct_duplication"] = float(parsed_json["duplication"]["rate"] * 100.0)
        except KeyError:
            log.debug(f"fasterp JSON did not have a 'duplication' key: '{s_name}'")
        ...
        try:
            for k in parsed_json["summary"]["after_filtering"]:
                self.fasterp_data[s_name][f"after_filtering_{k}"] = float(parsed_json["summary"]["after_filtering"][k])
        except KeyError:
            log.debug(f"fasterp JSON did not have a 'summary'-'after_filtering' keys: '{s_name}'")

        # Parse data required to calculate Pct reads surviving
        try:
            self.fasterp_data[s_name]["before_filtering_total_reads"] = float(
        ...
        try:
            self.fasterp_data[s_name]["pct_surviving"] = (
                self.fasterp_data[s_name]["filtering_result_passed_filter_reads"]
                / self.fasterp_data[s_name]["before_filtering_total_reads"]
            ) * 100.0
        ...
        try:
            for k in parsed_json["adapter_cutting"]:
                try:
                    self.fasterp_data[s_name][f"adapter_cutting_{k}"] = float(parsed_json["adapter_cutting"][k])
                except (ValueError, TypeError):
                    pass
        except KeyError:
            log.debug(f"fasterp JSON did not have a 'adapter_cutting' key, skipping: '{s_name}'")

        try:
            self.fasterp_data[s_name]["pct_adapter"] = (
                self.fasterp_data[s_name]["adapter_cutting_adapter_trimmed_reads"]
                / self.fasterp_data[s_name]["before_filtering_total_reads"]
            ) * 100.0
        except (KeyError, ZeroDivisionError) as e:
        ...
        try:
            ...
            for i, v in enumerate(parsed_json["duplication"]["histogram"]):
                self.fasterp_duplication_plotdata[s_name][i + 1] = (float(v) / float(total_reads)) * 100.0
        except KeyError:
        ...
        try:
            ...
            for i, v in enumerate(parsed_json["insert_size"]["histogram"]):
                if i <= max_i:
                    self.fasterp_insert_size_data[s_name][i + 1] = (float(v) / float(total_reads)) * 100.0
        except KeyError:
        ...
        for k in [
            ...
            try:
                for i, v in enumerate(parsed_json[k]["quality_curves"]["mean"]):
                    self.fasterp_qual_plotdata[k][s_name][i + 1] = float(v)
            except KeyError:
            ...
            try:
                for i, v in enumerate(parsed_json[k]["content_curves"]["GC"]):
                    self.fasterp_gc_content_data[k][s_name][i + 1] = float(v) * 100.0
                for i, v in enumerate(parsed_json[k]["content_curves"]["N"]):
                    self.fasterp_n_content_data[k][s_name][i + 1] = float(v) * 100.0
            except KeyError:
                log.debug(f"Content curve data {k} not found: {s_name}")

            # Overrepresented sequences
            try:
                self.fasterp_overrepresented_sequences[k][s_name] = parsed_json[k]["overrepresented_sequences"]
            except KeyError:
                log.debug(f"Overrepresented sequences data {k} not found: {s_name}")

        # Remove empty dicts
        if len(self.fasterp_data[s_name]) == 0:
            del self.fasterp_data[s_name]
        if len(self.fasterp_duplication_plotdata[s_name]) == 0:
            del self.fasterp_duplication_plotdata[s_name]
        if len(self.fasterp_insert_size_data[s_name]) == 0:
            del self.fasterp_insert_size_data[s_name]
        if len(self.fasterp_all_data[s_name]) == 0:
            del self.fasterp_all_data[s_name]
        ...
        # fasterp may have the same field or a different one
    ...
    def fasterp_general_stats_table(self, data_by_sample):
        """Take the parsed stats from the fasterp report and add it to the
    ...
    def fasterp_filtered_reads_chart(self):
        """Function to generate the fasterp filtered reads bar plot"""
        ...
        # Config for the plot
        pconfig = {
            "id": "fasterp_filtered_reads_plot",
            "title": "Fasterp: Filtered Reads",
            "ylab": "# Reads",
            "cpswitch_counts_label": "Number of Reads",
            "hide_zero_cats": False,
            "tt_decimals": 0,
        }
        return bargraph.plot(self.fasterp_data, keys, pconfig)

    def fasterp_read_qual_plot(self):
        """Make the read quality plot for Fasterp"""
        data_labels, pdata = self.filter_pconfig_pdata_subplots(self.fasterp_qual_plotdata, "Sequence Quality")
        pconfig = {
            "id": "fasterp-seq-quality-plot",
            "title": "Fasterp: Sequence Quality",
            "xlab": "Read Position",
    ...
    def fasterp_read_gc_plot(self):
        """Make the read GC plot for Fasterp"""
        data_labels, pdata = self.filter_pconfig_pdata_subplots(self.fasterp_gc_content_data, "Base Content Percent")
        pconfig = {
            "id": "fasterp-seq-content-gc-plot",
            "title": "Fasterp: Read GC Content",
            "xlab": "Read Position",
    ...
    def fasterp_read_n_plot(self):
        """Make the read N content plot for Fasterp"""
        data_labels, pdata = self.filter_pconfig_pdata_subplots(self.fasterp_n_content_data, "Base Content Percent")
        pconfig = {
            "id": "fasterp-seq-content-n-plot",
            "title": "Fasterp: Read N Content",
            "xlab": "Read Position",
            "ylab": "R1 Before filtering: Base Content Percent",
            "y_clipmax": 100,
            "y_minrange": 5,
    ...
    def fasterp_overrepresented_sequences_plot(self):
        cnt_by_sample = defaultdict(Counter)
        cnt_by_seq = Counter()
        pct_by_seq = Counter()
        samples_by_seq = defaultdict(set)

        for read_name, by_sample in self.fasterp_overrepresented_sequences.items():
            for s_name, by_seq in by_sample.items():
                for seq, count in by_seq.items():
                    cnt_by_seq[seq] += count
                    pct_by_seq[seq] += count / self.fasterp_data[s_name]["before_filtering_total_reads"]
        ...
        self.add_section(
            ...
            anchor="fasterp_overrepresented_sequences",
            ...
            plot=bargraph.plot(
                ...
                {
                    "id": "fasterp_overrepresented_sequences_plot",
                    "title": "Fasterp: Overrepresented sequences",
                    "cpswitch": False,
        ...
        top_n = getattr(config, "fasterp_config", {}).get("top_overrepresented_sequences", 20)
        ...
        self.add_section(
            ...
            anchor="fasterp_top_overrepresented_sequences",
            description=f"""
            ...
            plot=table.plot(
                ...
                headers={
                    ColumnKey("samples"): {
                        "title": "Reports",
                        "description": "Number of fasterp reports where this sequence is found as overrepresented",
                        "scale": "Greens",
                        "min": 0,
                        "format": "{:,d}",
                    },
                ...
                pconfig={
                    "namespace": self.name,
                    "id": "fasterp_top_overrepresented_sequences_table",
                    "title": "fasterp: Top overrepresented sequences",

...
function load_fastqc_passfails() {
  $(".fastqc_passfails").each(function (i, elem) {
    let key_value = JSON.parse(elem.innerHTML);
...
$(function () {
  ...
  let fastqc_modules = $(".fastqc_passfails").closest(".mqc-module-section");
  fastqc_modules.each(function () {
    let module_element = $(this);
    let module_key = module_element.attr("id").replace(/-/g, "_").replace("mqc_module_section_", "");
...
function fastqc_module(module_element, module_key) {
  ...
  let ypos = 0;
  let max_bp = 0;
  let current_single_plot = undefined;
  ...
  function fastqc_seq_content_heatmap() {
    ...
    let canvas = module_element.find("#fastqc_seq_heatmap")[0];
  ...
  module_element.find(".fastqc_per_base_sequence_content_plot").on("mqc_plotexport_data", function (e, cfg) {
    if (cfg["ft"] === "json") {
      let json_str = JSON.stringify(fastqc_seq_content[module_key], null, 2);
      let blob = new Blob([json_str], { type: "text/plain;charset=utf-8" });

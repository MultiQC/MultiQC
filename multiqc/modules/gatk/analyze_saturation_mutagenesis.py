...
def parse_gatk_analyze_saturation_mutagenesis(module: BaseMultiqcModule) -> int:
    """Find GATK AnalyzeSaturationMutagenesis logs and parse their data"""

    result = {}

    for file_handle in module.find_log_files("gatk/analyze_saturation_mutagenesis", filehandles=True):
        parsed_data = parse_read_counts_file(file_handle["f"])
        if len(parsed_data) > 1:
            if file_handle["s_name"] in result:
                log.debug(f"Duplicate sample name found! Overwriting: {file_handle['s_name']}")
            module.add_data_source(file_handle, section="analyze_saturation_mutagenesis")
            result[file_handle["s_name"]] = parsed_data

    # Filter to strip out ignored sample names
    result = module.ignore_samples(result)

    n_reports_found = len(result)
    ...
    module.write_data_file(result, "multiqc_gatk_analyze_saturation_mutagenesis")

    gatk_analyze_saturation_mutagenesis_table(module, result)
    # Add plots
    gatk_analyze_saturation_mutagenesis_plot_reads(module, result)
    gatk_analyze_saturation_mutagenesis_plot_base_calls(module, result)

    return n_reports_found


def parse_read_counts_file(file_handle):
...
def gatk_analyze_saturation_mutagenesis_plot_reads(module, data):
...
def gatk_analyze_saturation_mutagenesis_plot_base_calls(module, data):
...
def gatk_analyze_saturation_mutagenesis_table(module, data):

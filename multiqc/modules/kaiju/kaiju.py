...
class MultiqcModule(BaseMultiqcModule):
    ...
    def kaiju_stats_table(self):
        ...
        for s_name, d in self.kaiju_sample_total_readcounts.items():
            tdata[s_name] = {}
            tdata[s_name]['% Unclassified'] = self.kaiju_sample_unclassified[s_name] * 100 / self.kaiju_sample_total_readcounts[s_name]
            if s_name in self.kaiju_data[general_taxo_rank.lower()]:
                tdata[s_name]['assigned'] = self.kaiju_sample_total_readcounts[s_name] - self.kaiju_sample_unclassified[s_name] - self.kaiju_data[general_taxo_rank.lower()][s_name]["cannot be assigned"]["count"]
                tdata[s_name]['% Assigned'] = tdata[s_name]['assigned'] * 100 / self.kaiju_sample_total_readcounts[s_name]
            else:

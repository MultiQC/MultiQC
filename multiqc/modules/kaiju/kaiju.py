...
class MultiqcModule(BaseMultiqcModule):
    ...
    def parse_kaiju2table_report(self, report):
        ...
        for s_name in parsed_data :
            parsed_data[s_name]['percentage_assigned'] = (float(parsed_data[s_name]["assigned"]) / float(parsed_data[s_name]["assigned"] + parsed_data[s_name]["cannot_be_assigned"] + parsed_data[s_name]["unclassified"])) * 100.0
    ...
    def kaiju_stats_table(self):
        ...
        if len(taxo_ranks) >= 1 and "phylum" in taxo_ranks:
            ...
            general_taxo_rank = self.kaiju_data.keys().sorted()[0]
    ...
    def kaiju_taxo_rank_plot (self):
        ...
        description = '<p>This plot shows the proportion of reads assigned, unclassified or not classified at different taxonomic rank.</p>'
        pconfig = {
            'id': 'kaiju_taxo_rank_plot',
            'title': 'Kaiju: reads classification',
        ...
        for rank in self.taxonomic_ranks :
            if rank in self.kaiju_data :
                pconfig['data_labels'].append( {'name': rank, 'ylab': 'Number of Reads at {} rank'.format(rank) })
        ...
        self.add_section (
            name = 'Reads classification at different taxonomic level',

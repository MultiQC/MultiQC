...
class MultiqcModule(BaseMultiqcModule):
    ...
    2. **Sample stats file** ('\*.L{1,2,3,4}.mgikit.sample_stats'): Sample statistics for each lane like yield, quality scores, cluster count
    1. **General info file** ('\*.L{1,2,3,4}.mgikit.general'): Rounded up sample stats, but also includes lane-level stats
    2. **General info file** ('\*.L{1,2,3,4}.mgikit.info'): Matching indexes within the data generated by a specific lane
    3. **Undetermined barcodes file** ('\*.L{1,2,3,4}.mgikit.undetermined_barcode'): Barcodes that did not match with any sample.
    4. **Ambiguous barcodes file** ('\*.L{1,2,3,4}.mgikit.ambiguous_barcode'): Barcodes that match with multiple samples. This situation can happen when setting a high mismatch threshold.

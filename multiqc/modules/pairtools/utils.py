...
def edges_to_intervals(edges) -> list[tuple[int, int | None]]:
    """
    ...
    return list(itertools.pairwise(_edges))
...
def cum_sums_to_range_sums(cum_sums, total) -> tuple[int, ...]:
    ...
    return tuple((_h - _l) for _h, _l in itertools.pairwise(_cum_c_sums))


def parse_dist_range(dist_range) -> tuple[int, int | None]:
    """

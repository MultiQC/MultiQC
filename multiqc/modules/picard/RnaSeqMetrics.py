...
def parse_reports(self):
    ...
    # Set up vars
    self.picard_rna_seq_metrics_data = dict()
    ...
    for f in self.find_log_files("picard/rnaseqmetrics", filehandles=True):
        ...
        for l in f["f"]:
            ...
            if s_name is not None:
                if "rnaseqmetrics" in l.lower() and "## METRICS CLASS" in l:
                    if s_name in self.picard_rna_seq_metrics_data:
                        log.debug("Duplicate sample name found in {}! Overwriting: {}".format(f["fn"], s_name))
                    self.picard_rna_seq_metrics_data[s_name] = dict()
                    ...
                    for i, k in enumerate(keys):
                        ...
                        try:
                            self.picard_rna_seq_metrics_data[s_name][k] = float(vals[i])
                        except ValueError:
                            self.picard_rna_seq_metrics_data[s_name][k] = vals[i]
                        except IndexError:
                            pass  # missing data
                    # Calculate some extra numbers
                    if "PF_BASES" in keys and "PF_ALIGNED_BASES" in keys:
                        self.picard_rna_seq_metrics_data[s_name]["PF_NOT_ALIGNED_BASES"] = (
                            self.picard_rna_seq_metrics_data[s_name]["PF_BASES"]
                            - self.picard_rna_seq_metrics_data[s_name]["PF_ALIGNED_BASES"]
                        )
        ...
        for key in list(self.picard_rna_seq_metrics_data.keys()):
            if len(self.picard_rna_seq_metrics_data[key]) == 0:
                self.picard_rna_seq_metrics_data.pop(key, None)
    ...
    self.picard_rna_seq_metrics_data = self.ignore_samples(self.picard_rna_seq_metrics_data)

    if len(self.picard_rna_seq_metrics_data) == 0:
        return 0
    ...
    self.write_data_file(self.picard_rna_seq_metrics_data, "multiqc_picard_RnaSeqMetrics")
    ...
    self.general_stats_addcols(self.picard_rna_seq_metrics_data, GenStatsHeaders)
    ...
    warn_rrna = ""
    rrna_missing = []
    for s_name, metrics in self.picard_rna_seq_metrics_data.items():
    ...
    self.add_section(
        ...
        plot=bargraph.plot(self.picard_rna_seq_metrics_data, bg_cats, pconfig),
    )
    ...
    pdata = dict()
    for s_name, d in self.picard_rna_seq_metrics_data.items():
    ...
    return len(self.picard_rna_seq_metrics_data)

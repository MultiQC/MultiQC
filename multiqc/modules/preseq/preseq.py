    ...
    def _parse_real_counts(self, sample_names):
        ...
        if real_counts_file_raw is not None:
            try:
                ...
                log.error("Error loading real counts file {}: {}".format(
                    real_counts_file_name, str(e)))
            else:
                log.debug("Found {} matching sets of counts from {}".format(
                    len(real_counts_total), real_counts_file_name))

...
def parse_reports(self):
    ...
    for f in self.find_log_files('sentieon/insertsize', filehandles=True):
        ...
        for l in f['f']:
            ...
            if s_name is not None and in_hist is True:
                try:
                    ...
                    self.sentieon_insertSize_samplestats[s_name][
            ...
            if s_name is not None:
                if 'InsertSizeMetricAlgo' in l and '#SentieonCommandLine' in l:
                    ...
                    self.add_data_source(
                        f, s_name, section='InsertSizeMetrics')
                    keys = f['f'].readline().strip("\n").split("\t")
                    vals = f['f'].readline().strip("\n").split("\t")
                    self.sentieon_insertSize_samplestats[s_name] = {
                        'total_count': 0, 'meansum': 0, 'total_pairs': 0}
                    ...
                    while len(vals) == len(keys):
                        ...
                        self.sentieon_insertSize_data[rowkey]['SAMPLE_NAME']\
                            = s_name
                        for i, k in enumerate(keys):
                            try:
                                self.sentieon_insertSize_data[rowkey][k]\
                                     = float(vals[i])
                            except ValueError:
                                try:
                                    self.sentieon_insertSize_data[rowkey][k] =\
                                         float(vals[i].replace(',', '.'))
                                    log.debug(
                                ...
                                pass  # missing data
                        # Add to mean sums
                        rp = self.sentieon_insertSize_data[rowkey][
                            'READ_PAIRS']
                        mis = self.sentieon_insertSize_data[rowkey][
                            'MEAN_INSERT_SIZE']
                        self.sentieon_insertSize_samplestats[s_name][
                            'meansum'] += (rp * mis)
                        self.sentieon_insertSize_samplestats[s_name][
        ...
        for s_name in list(self.sentieon_insertSize_histogram.keys()):
            if len(self.sentieon_insertSize_histogram[s_name]) == 0:
                ...
                log.debug("Ignoring '{}' histogram as no data parsed".format(
                    s_name))

    # Calculate summed mean values for all read orientations
    for s_name, v in self.sentieon_insertSize_samplestats.items():
        self.sentieon_insertSize_samplestats[s_name]['summed_mean'] = v[
            'meansum'] / v['total_pairs']
    ...
    self.sentieon_insertSize_data\
        = self.ignore_samples(self.sentieon_insertSize_data)

    if len(self.sentieon_insertSize_data) > 0:

        # Write parsed data to a file
        self.write_data_file(
            self.sentieon_insertSize_data, 'multiqc_sentieon_insertSize')
        ...
        if len(self.sentieon_insertSize_histogram) > 0:
            ...
            try:
                insertsize_smooth_points = int(
                    config.sentieon_config['insertsize_smooth_points'])
                log.debug("Custom Sentieon insert size smoothing:\
            ...
            self.add_section(
                ...
                plot=linegraph.plot(
                    [self.sentieon_insertSize_histogram, data_percent],
                    pconfig)
            )

...
class gather:
    def parse_gather(self):
        ...
        self.gather_total_pct_unique_to_query = dict()
        ...
        for s_name, data in self.gather_raw_data.items():
            for row in data:
                ...
                if match_name not in self.gather_total_pct_unique_weighted:
                    ...
                    self.gather_total_pct_unique_to_query[match_name] = 0
                self.gather_total_pct_unique_weighted[match_name] += row["pct_unique_weighted"]
                self.gather_total_pct_unique_to_query[match_name] += row["pct_unique_to_query"]
    ...
    def load_gather(self, f):
        ...
        for line in gatherr:    
            row = {
                ...
                "pct_orig_query" : float(line['f_orig_query']) * 100,
                "pct_unique_to_query" : float(line['f_unique_to_query']) * 100,

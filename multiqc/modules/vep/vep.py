...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        ...
        super(MultiqcModule, self).__init__(
            ...
            info="VEP determines the effect of your variants on genes, transcripts, and protein sequence, as well as regulatory regions.",
        )

        self.vep_data = dict()
        for f in self.find_log_files("vep", filehandles=True):
        ...
        self.general_stats_addcols(self.vep_data, headers)

        # Write parsed report data to a file
        self.write_data_file(self.vep_data, "multiqc_samblaster")

        log.info("Found {} reports".format(len(self.vep_data)))
    ...
    def add_barplot(self):
        ...
        self.add_section(plot=bargraph.plot(self.vep_data, cats, pconfig))
    ...
    def parse_samblaster(self, f):
        ...
        if len(data) > 0:
            if s_name in self.vep_data:
                log.debug("Duplicate sample name found in {}! Overwriting: {}".format(f["fn"], s_name))
            self.add_data_source(f, s_name)
            self.vep_data[s_name] = data

...
$(function () {
  ...
  $(".btn-ai-generate-summary").click(async function (e) {
    ...
    let aiProviderTitle = button.data("ai-provider-title");
    let aiModel = button.data("ai-model");
    let aiToken = button.data("ai-token");
    let seqeraApiUrl = button.data("seqera-api-url"); // if provider is Seqera
    let seqeraWebsite = button.data("seqera-website"); // if provider is Seqera, for "continue in Seqera" button
...
`;
    ...
    const summaryContainer = $("#" + sectionAnchor + "_ai_summary .ai-summary-main-text");
    ...
    await streamGeneration(seqeraApiUrl, button, summaryContainer, systemPrompt, reportData, aiModel, aiToken, [
      "multiqc-plot-summary",
...
async function streamGeneration(
  ...
  while (true) {
    ...
    targetDiv.html(markdownToHtml(receievedText));
...
async function generateMoreHandler(event) {
  ...
  **Recommendations**
  ...
  try {
    streamGeneration(seqeraApiUrl, button, responseDiv, systemPrompt, contentBase64, aiModel, aiToken, [
      "multiqc",
      "multiqc-generate-more",
    ]);
    ...
    responseDiv.after($("<span style='color: red; font-style: italic;'>Error while generating details</span>"));
  }

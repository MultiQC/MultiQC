...
from multiqc.templates.plotly.plots.plot import AbstractPlot
...
def plot(
    ...
    """
    A helper function for back-compatibility with the old plotting functions.
    In the future, when refactored for interactive environments, the classes
    will be used directly.
    """
    return Plot(pconfig).add_to_report(
        data_by_cat_lists,
        samples_lists,
    )
...
class Plot(AbstractPlot):
    ...
    def add_to_report(
        ...
        if config.plots_force_flat:
            return self._add_to_report_flat(
                data_by_cat_lists,
                samples_lists,
            )
        else:
            return self._add_to_report_interactive(
                data_by_cat_lists,
                samples_lists,
            )
    ...
    def _add_to_report_interactive(
        ...
        btn_tmpl = (
            f'<button class="{{cls}} btn btn-default btn-sm {{active}}" data-target="{self.id}">{{label}}'
            f"</button> \n"
        )
        ...
        if len(data_by_cat_lists) > 1:
            ...
            for k, p in enumerate(data_by_cat_lists):
                ...
                try:
                    name = self.data_labels[k]["name"]
                except (TypeError, KeyError):
                    try:
                        name = self.data_labels[k]
                    except KeyError:
                        name = k + 1
                try:
                    ymax = f'data-ymax="{self.data_labels[k]["ymax"]}"'
                except:
                    ymax = ""
        ...
        report.plot_data[self.id] = {
            ...
            "layout": self._layout().to_plotly_json(),
    ...
    def _add_to_report_flat(
        ...
        for k in range(len(data_by_cat_lists)):
            try:
                ...
                name = k + 1
            pid = report.save_htmlid(f"{self.id}_{name}", skiplint=True)
            pids.append(pid)
        ...
        if self.add_pct_tab and not config.simple_output:
            html += f'<div class="btn-group mpl_switch_group mqc_mplplot_bargraph_setcountspcnt"> \n\
                <button class="btn btn-default btn-sm {self.c_active} counts">{self.c_label}</button> \n\
                <button class="btn btn-default btn-sm {self.p_active} pcnt">{self.p_label}</button> \n\
            </div> '
            if len(data_by_cat_lists) > 1:
                html += " &nbsp; &nbsp; "
        ...
        for pidx, (samples, data_by_cat) in enumerate(zip(samples_lists, data_by_cat_lists)):
            ...
            for view in views:
                ...
                if getattr(get_template_mod(), "base64_plots", True) is True:
                    ...
                    img_buffer.close()
                    html += f'<div class="mqc_mplplot" id="{pid}"{hide_div}><img src="data:image/png;base64,{b64_img}" /></div>'
